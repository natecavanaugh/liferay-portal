...
<c:if test="<%= !group.isControlPanel() && !group.isUserPersonalPanel() && userSetupComplete %>">
	<ul class="control-menu" data-namespace="<portlet:namespace />" id="<portlet:namespace />controlMenu">
		<li class="left pull-left">
			<ul>
				<li>
					<a class="control-menu-icon sidenav-toggler" href="#1" id="mySidenavToggleId"><span class="icon-align-justify icon-monospaced"></span></a>
				</li>
			</ul>
		</li>
			...
			<ul>
				...
				<c:if test="<%= user.isSetupComplete() || themeDisplay.isImpersonated() %>">
					...
					<c:if test="<%= hasMessages %>">
						<div class="hide" id="<portlet:namespace />infoContainer">
						...
						<aui:script sandbox="<%= true %>">
							$('#<portlet:namespace />infoButton').popover(
								{
									html: true,
									content: $('#<portlet:namespace />infoContainer').html(),
									placement: 'top'
								}
							);
						</aui:script>
