AUI.add(
	...
	function(A) {
		var Lang = A.Lang,
			...
			SELECTOR_NOTICE_INLINE_EDIT = '.notice-inline-edit',
			SELECTOR_NOTICE_INLINE_EDIT_TEXT = '.notice-inline-edit-text',
				...
				prototype: {
					initializer: function(config) {
						...
						instance._eventHandles = [
							...
							editor.on('saveContent', A.bind(instance._saveContent, instance)),
					...
					_afterSaveContentSuccess: function(responseData, autosaved) {
						...
						boundingBox.one(SELECTOR_NOTICE_INLINE_EDIT_TEXT).html(message);
					...
					_getEditNotice: function() {
						...
						var editNoticeNode = A.one(SELECTOR_NOTICE_INLINE_EDIT);
						...
						if (!editNoticeNode) {
							...
							editNotice.get(BOUNDING_BOX).one(SELECTOR_NOTICE_INLINE_EDIT).setData(NOTICE_INSTANCE, editNotice);

							instance._attachCloseListener();
						}
						else {
							editNotice = editNoticeNode.getData(NOTICE_INSTANCE);
						}
					...
					_restoreContent: function() {
						...
						var originalContentNode = A.one('#' + instance.get(EDITOR_NAME) + instance.CKEDITOR_SUFFIX);
