		...
		function(A) {
			...
			var ControlPanel = {
				...
				_panelHolderAccordionDisable: function() {
					...
					var handler = A.on(function(e) {
						return new A.Do.Prevent();
					}, component, 'headerEventHandler');
					...
					instance._panelHolder.transition(
						{
							...
							on: {
								start: function() {
									...
									instance._panelHolder.one('.search-panels-bar').setStyle('display', 'none');
								},

								end: function() {
									body.addClass(CSS_PANELS_MINIMIZED).removeClass('animating');

									instance._panelHolder.setStyle('width','');

									instance._panelHolder.one('.search-panels-bar').setStyle('display', '');

									instance._panelPageBody.setStyle('width','');
								}
							}
						}
					);
					...
					instance._panelHolder.transition(
						{
							...
							on: {
								start: function() {
									instance._panelPageBody.setStyle('width','74%');
								},
				...
				_panelHolderHandleClick: function(event) {
					...
					var accordionGroup = element.ancestor('.accordion-group');
					...
					element.once('clickoutside', function() {
						accordionGroup.removeClass('open');
					});
				},
				...
				_renderUI: function() {
					...
					if (panelHolder) {
						...
						panelPageBody.prepend(togglePanels);
