		...
		<c:if test="<%= Validator.isNotNull(types) %>">
			<aui:field-wrapper label="social-bookmarks">
				...
				%>
					...
					<aui:input checked="<%= ArrayUtil.contains(typesArray, type) %>" includeHiddenField="<%= false %>" id='<%= "socialBookmarksTypestype" + type %>' label="<%= type %>" name="preferences--socialBookmarksTypes--" type="checkbox" value="<%= type %>" />
