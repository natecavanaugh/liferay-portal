...
<div class="wiki-body">
	<div class="wiki-info">
		<span class="stats">${assetEntry.getViewCount()} ${macro.language("views")}</span> |

		<span class="date">${macro.language("last-modified")} ${dateUtil.getDate(entry.getModifiedDate(), "dd MMM yyyy - HH:mm:ss", locale)}</span>

		<span class="author">${macro.language("by")} ${htmlUtil.escape(portalUtil.getUserName(entry.getUserId(), entry.getUserName()))}</span>
	</div>
...
<#if (childPages?has_content)>
	<div class="child-pages">
		<h2>${macro.language("children-pages")}</h2>
		...
		<table class="taglib-search-iterator">
			<tr class="portlet-section-header results-header">
				...
				<th>
					${macro.language("last-modified")}
				</th>
				<th>
					${macro.language("ratings")}
				</th>
				<th>
					${macro.language("views")}
				</th>
			</tr>
			...
			<#list childPages as childPage>
				<tr class="results-row">
					...
					<td>
						<a href="${viewPageURL}">${dateUtil.getDate(childPage.getModifiedDate(),"dd MMM yyyy - HH:mm:ss", locale)} ${macro.language("by")} ${htmlUtil.escape(portalUtil.getUserName(childPage.getUserId(), childPage.getUserName()))}</a>
					</td>
					<td>
